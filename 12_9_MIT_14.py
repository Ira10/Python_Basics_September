...
def generate_word_dict(song):
    ...
    * values are the frequency of each key in song
    ...
    words_list = song_words.split() ## converting string to list of words

    # print(words_list) # ['rah', 'rah', 'ah', 'ah', 'ah', 'rom', 'mah', 'ro', 'mah', 'mah']

    word_dict = {}

    for w in words_list:  ## can iterate over list of words in a string
        if w in word_dict:
            # seen word again, so add one to frequency
            word_dict[w] += 1
        else:
            # first time seeing word, create a dict entry with freq 1
            word_dict[w] = 1
...
# print(word_dict)   # {'rah': 2, 'ah': 3, 'rom': 1, 'mah': 3, 'ro': 1}
